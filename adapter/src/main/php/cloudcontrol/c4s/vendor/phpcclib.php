...
class API {
     ...
     * @throws CCException
     */
    public function deployment_removeAddon($applicationName, $deploymentName, $addonName) {
        $resource = sprintf('/app/%s/deployment/%s/addon/%s', $applicationName, $deploymentName, $addonName);
        $data = array('addon' => $addonName);
        return $this->_executeDelete($resource, $data);
    }
...
//     * @throws CCException
//     */
    public function deployment_getAddonDetails($applicationName, $deploymentName, $addonName) {
        $resource = sprintf('/app/%s/deployment/%s/addon', $applicationName, $deploymentName);
        $data = array('addon' => $addonName);
        return $this->_executeGet($resource, $data);
    }
